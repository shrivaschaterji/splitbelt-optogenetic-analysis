...
# Inputs
laser_event = 'swing'
path = 'C:\\Users\\Ana\\Documents\\PhD\\Projects\\Online Stimulation Treadmill\\270323 final test with split and narrow threshold\\'
animal = 'MC16946HR'
session = 1
Ntrials = 20
stim_start = 16
...
for p in range(np.shape(param_sym)[0] - 1):
    ...
    rectangle = plt.Rectangle((stim_start - 0.5, min(param_sym_bs[p, :, :].flatten())), stim_duration,
...
[stride_idx, trial_continuous, sl_time, sl_values] = loco.param_continuous_sym(param_sl, st_strides_trials, trials, 'FR', 'FL', 1, 0)
...
if print_plots:
    if not os.path.exists(path_save):
        os.mkdir(path_save)
    plt.savefig(path_save + animal_list[a] + 'sl_sym_continuous', dpi=96)